...
impl EpubBook<File> {
    ...
    pub fn open_with_temp_storage<EP: AsRef<Path>, TP: AsRef<Path>>(
        ...
        epub_temp_trace!("[EPUB-TEMP] zip_ready");
        ...
        let (navigation, navigation_loaded) = if config.lazy_navigation {
            epub_temp_trace!("[EPUB-TEMP] navigation_deferred");
            (None, false)
        } else {
            (
                parse_navigation(
        ...
        if navigation_loaded {
            epub_temp_trace!("[EPUB-TEMP] navigation_loaded");
        }
...
impl<R: Read + Seek> EpubBook<R> {
    ...
    pub fn from_reader_with_config(reader: R, config: OpenConfig) -> Result<Self, EpubError> {
        ...
        let metadata = extract_metadata_with_limits(&container, &opf, &options.metadata_limits)?;
        let spine = crate::spine::parse_spine_with_limits(&opf, &options.metadata_limits)?;
        validate_open_invariants(&metadata, &spine, options.validation_mode)?;
        let (navigation, navigation_loaded) = if config.lazy_navigation {
            (None, false)
        } else {
    ...
    pub fn cover_image_ref_with_options(
        ...
        let mut doc_buf = Vec::with_capacity(8);
...
mod tests {
    ...
    fn test_reading_session_resolve_locator_and_progress() {
        ...
        let nav = Navigation {
            ...
            page_list: Vec::with_capacity(8),
            landmarks: Vec::with_capacity(8),
        };
